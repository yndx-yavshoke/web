...
import { generateRandomEmail, generateRandomPassword, generateRandomAge } from "../utils/dataGenerator";
    ...
    test.describe('Проверка элементов страницы регистрации', () => {
        test('register_page_should_contain', async ({ registerPage }) => {
            ...
            await test.step('Проверить элементы формы', async () => {
                ...
                await expect.soft(registerPage.emailInput, 'Плейсхолдер email должен быть Email').toHaveAttribute('placeholder', 'Email');
                await expect.soft(registerPage.passwordInput, 'Поле пароля должно быть видно').toBeVisible();
                await expect.soft(registerPage.passwordInput, 'Плейсхолдер пароля должен быть Пароль').toHaveAttribute('placeholder', 'Пароль');
                await expect.soft(registerPage.ageInput, 'Поле возраста должно быть видно').toBeVisible();
                await expect.soft(registerPage.ageInput, 'Плейсхолдер возраста должен быть Возраст').toHaveAttribute('placeholder', 'Возраст');
                await expect.soft(registerPage.registerButton, 'Кнопка регистрации должна быть видимой').toBeVisible();
                await expect.soft(registerPage.registerButton, 'Текст кнопки регистрации должен быть Зарегистрироваться').toHaveText('Зарегистрироваться');
                await expect.soft(registerPage.toLoginButton, 'Кнопка возврата должна быть видимой').toBeVisible();
                await expect.soft(registerPage.toLoginButton, 'Текст кнопки возврата должен быть Назад').toHaveText('Назад');
            });
        });
    });