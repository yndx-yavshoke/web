...
test.describe('Проверка элементов главной страницы', () => {
    test('main_page_should_contain', async ({ mainPage }) => {
        ...
        await test.step('Проверить элементы главной страницы', async () => {
            ...
            await expect.soft(mainPage.input, 'Плейсхолдер поля ввода email должен быть Введите email').toHaveAttribute('placeholder', 'Введите email');
            await expect.soft(mainPage.checkButton, 'Кнопка проверки должна быть видимой').toBeVisible();
            await expect.soft(mainPage.checkButton, 'Текст кнопки проверки должен быть "Я в шоке?"').toHaveText('Я в шоке?');
            await expect.soft(mainPage.toLoginButton, 'Кнопка входа должна быть видимой').toBeVisible();
            await expect.soft(mainPage.toLoginButton, 'Текст кнопки входа должен быть В шок').toHaveText('В шок');
        });
    });
});