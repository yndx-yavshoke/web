...
test.skip('adult kotik state', async ({ profilePage }) => {
    await test.step('Мокаем API для состояния взрослого котика', async () => {
        await profilePage.page.route('https://api.yavshok.ru/experiments', (route) => {
            route.fulfill({
                status: 200,
    ...
    await test.step('Отображается статус "Ты взрослый котик"', async () => {
        await expect(profilePage.status).toContainText('Ты взрослый котик');
    });
})
