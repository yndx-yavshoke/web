describe('Edit Profile Button test', () => {
    ...
    it('If edit button correct', async ({ browser }) => {
        await authenticateUser(browser);
        await browser.pause(800);

        const btn = await browser.$('[data-testid="user-edit-profile-button"]');
        await btn.waitForDisplayed({ timeout: 5000 });

        await browser.assertView('edit-btn-view', '[data-testid="user-edit-profile-button"]');
    });
});
